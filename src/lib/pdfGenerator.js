...
export const generateGuestVisitPDF = async (guestData) => {
  ...
  autoTable(doc, {
    ...
    body: [
      ...
      ['District:', guestData.district || '—', 'Donation:', guestData.donation_amount ? `Rs. ${Number(guestData.donation_amount).toLocaleString('en-IN')}` : '—'],
      ['State/Country:', guestData.is_international ? guestData.country : guestData.state, '', ''],
    ],
    theme: 'plain',
